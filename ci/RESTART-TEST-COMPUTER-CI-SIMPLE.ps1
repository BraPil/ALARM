# ADDS25 Test Computer CI Restart Script - SIMPLE VERSION
# Purpose: Restart test computer CI monitoring after system freeze/crash
# Environment: Test Computer (wa-bdpilegg)
# Date: September 2, 2025

Write-Host "ADDS25 Test Computer CI Restart - SIMPLE VERSION" -ForegroundColor Cyan
...
Write-Host "SUCCESS: Current directory: $(Get-Location)" -ForegroundColor Green
...
try {
    ...
    Write-Host "WARNING: Error pulling changes: $($_.Exception.Message)" -ForegroundColor Red
...
if (Test-Path $adds25Path) {
    Write-Host "SUCCESS: ADDS25 directory found: $adds25Path" -ForegroundColor Green
    ...
    foreach ($launcher in $launchers) {
        if (Test-Path $launcher) {
            Write-Host "SUCCESS: Launcher found: $(Split-Path $launcher -Leaf)" -ForegroundColor Green
        } else {
            Write-Host "WARNING: Launcher missing: $(Split-Path $launcher -Leaf)" -ForegroundColor Yellow
        }
    }
} else {
    Write-Host "ERROR: ADDS25 directory not found: $adds25Path" -ForegroundColor Red
}
